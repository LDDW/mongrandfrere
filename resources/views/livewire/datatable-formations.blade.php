<x-datatable>
    ...
    <x-slot name="modal">
        <x-confirmation-modal wire:model="confirmingDeletion" >
            ...
            <x-slot name="footer">
                ...
                <form action="{{ Route('admin.formation.destroy', ['formation' => $IdBeingDeleted]) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <x-datatable-button class="ml-2" label="SUPPRIMER LA FORMATION"  color="red-danger" />
                </form>
            ...
            @foreach ($formations as $i => $formation)
                <tr class="h-12 {{$i % 2 == 0 ? 'bg-gray-50' :''}} border-t border-gray-200 text-gray-900 flex flex-row">
                    ...
                    <td class="w-full min-w-[128px] pl-2 h-12 flex items-center gap-2">
                        ...
                        <button wire:click="deleteModal({{ $formation->id }})" class="text-gray-100 flex items-center justify-center w-12 h-8 rounded-sm bg-red-400 hover:bg-red-500" >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
    ...
    <x-slot name="pages">
        @if($formations->lastPage() > 1)<div class="paginate_bar">{{ $formations->links() }}</div>@endif
    </x-slot>
</x-datatable>